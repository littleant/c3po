...
//    }
  ...
  public static BasicGraph getGraph(String property) {
    ...
    BasicGraph g = null;
    String property1 = form.get("property1");
    ...
    if (property1 == null) {
    	...
    	if (alg1 == null) {
    		g1 = getOrdinalGraph(filter, property1);
    	} else {
  	      	g1 = getNumericGraph(filter, property1, alg1, form.get("width1"));
    	}
  ...
  public static BasicGraph joinGraphs(Graph g1, Graph g2) {
	  ...
	  for (int i = 0; i < g1.getKeys().size(); i++) {
		  // e.g. valid (true, false, unknown)
		  
		  // find dimension for bubble
		  Filter filterBackup = Application.getFilterFromSession();
		  Filter filter = filterBackup;
		  // e.g. format
		  addFromFilter(filter, g1.getProperty(), g1.getKeys().get(i));
		  Graph gr = getGraph(filter, g2.getProperty());
		  
		  // make sure the index (y-axis) in the chart is always the right key, by mapping the string-value to an index
		  List<String> rKeys = new LinkedList<String>();
		  Map<String,Double> rValues = new HashMap<String,Double>();
		  for (int j = 0; j < g2.getKeys().size(); j++) {
			  rKeys.add(g2.getKeys().get(j));
			  rValues.put(g2.getKeys().get(j), 0D);
		  }
		  
		  for (int j = 0; j < gr.getKeys().size(); j++) {
			  rValues.put(gr.getKeys().get(j), Double.parseDouble(gr.getValues().get(j)));
		  }
